...
function keyFor(dividend, divisor) {
  return `${dividend}d${divisor}`;
}

// Division: makeFact(quotient, divisor) → { a: dividend, b: divisor, answer: quotient }
function makeFact(quotient, divisor) {
...
function buildNewStatus(level) {
  ...
  for (let n = 0; n <= maxN; n++) {
    // n is the quotient, level is the divisor → dividend = n * level
...
function pickRandomRemainingNew(level, statusObj) {
  ...
  const [dividendStr, divisorStr] = pickKey.split("d");
  const dividend = Number(dividendStr);
  const divisor = Number(divisorStr);
  const quotient = dividend / divisor;
  return makeFact(quotient, divisor);
}

function pickRandomAny(level) {
  const quotient = randomInt(0, level);
  const divisor = randomInt(1, level);
  return makeFact(quotient, divisor);
}

function pickEasierLevelQuestion(level) {
  // Pick a random fact from easier levels only (1 to level-1)
  if (level <= 1) {
    // No easier level, just pick from current level
    return pickRandomAny(level);
  }
  const quotient = randomInt(0, level - 1);
  const divisor = randomInt(1, level - 1);
  return makeFact(quotient, divisor);
}
...
export default function App() {
  ...
  function popDueReviewItem() {
    ...
    // Reconstruct division fact from stored dividend (a) and divisor (b)
    const quotient = due.a / due.b;
    return { fact: makeFact(quotient, due.b), hidden: !!due.hidden };
  }

  function classifyCorrection(fact, currentLevel) {
    // For division, the divisor (fact.b) is the level number
    return fact.b === currentLevel ? 'current' : 'lower';
  }
  ...
  function pickFromCorrections(correctionsList) {
    ...
    // Reconstruct division fact from stored dividend (a) and divisor (b)
    const quotient = pick.a / pick.b;
    return makeFact(quotient, pick.b);
  }
  ...
  function chooseNonBackToBack(getCandidateFn, nextLevel) {
    ...
    if (anyNew) {
      const [dividendStr, divisorStr] = anyNew.split("d");
      const dividend = Number(dividendStr);
      const divisor = Number(divisorStr);
      const quotient = dividend / divisor;
      return makeFact(quotient, divisor);
  ...
  function getNextQuestion(slotOverride = null) {
    ...
    // Level 1 special case: no lower level, just repeat corrections
    if (level === MIN_LEVEL) {
  ...
  function submit() {
    ...
    if (user === question.answer) {
      ...
      if (!didGraduate) {
        // Toggle slot (except for lowest level) and get next question
        if (level !== MIN_LEVEL) {
    ...
    return (
      <div className="wrap">
        <div className="centerTop">
          ...
          <div className="gameTitle">Division Memory</div>
        </div>

        <div className="card homeCard centerCard">
          <p className="sub centerText">
            Pick your starting level (the divisor).
          </p>
      ...
      <div className="wrap">
        ...
        <div className="card centerCard mainCard gameOver">
          ...
          <div className="celebrateText">
            You mastered levels {MIN_LEVEL}–{MAX_LEVEL}. Incredible work!
          </div>
          <button
  ...
  const checklistItems = newKeys
    .map((k) => {
      const [dividendStr, divisorStr] = k.split("d");
      const dividend = Number(dividendStr);
      const divisor = Number(divisorStr);
      const quotient = dividend / divisor;
      return { k, dividend, divisor, quotient, done: newStatus[k] === true };
    })
    .sort((x, y) => x.quotient - y.quotient);
  ...
  return (
    <div className="wrap">
      <div className="centerTop">
        <img className="logoTop" src={logo} alt="Kokoro Tutoring" />
        <div className="gameTitle">Division Memory</div>
        <div style={{ fontSize: '10px', color: '#999', marginTop: '4px' }}>v:DV1</div>
      </div>
